import ErrorNotificationService, { ErrorNotification, ErrorSeverity } from "../../../services/ErrorNotificationService";
...
test("fetch throws error", async () => {
  ...
  const errors = ErrorNotificationService.getAllNotifications();
  expect(errors).toHaveLength(1);
  expect(errors[0].message).toBe("Error while fetching some-endpoint: Network error occurred");
})
